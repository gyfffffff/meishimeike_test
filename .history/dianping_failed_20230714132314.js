// 引入必须模块
var fs = require('fs')   // 页面存储
var Request = require('request')  // 发送请求
var Cheerio = require('cheerio')  // 解析
var Iconv = require('iconv-lite') // utf8格式转化
require('date-utils')  // 日期解析

// var Encoding = "utf8"
// var seedURL = 'https://github.com/search?q=ChatGPT&type=repositories'
var seedURL = 'https://www.dianping.com/shanghai/ch10'

var headers = {
    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/114.0.0.0 Safari/537.36',
    'Cookie': 'navCtgScroll=100; navCtgScroll=0; fspop=test; cy=1; cye=shanghai; s_ViewType=10; _lxsdk_cuid=1893a333fe9c8-03d5e50256d40d-26031d51-e1000-1893a333fe9c8; _lxsdk=1893a333fe9c8-03d5e50256d40d-26031d51-e1000-1893a333fe9c8; _hc.v=c0666d72-45b3-e7a3-cf3e-4cab134fae84.1688898587; WEBDFPID=75yuy4361w105955zx53w1103295yw698106z6646129795885490x9w-2004258590093-1688898583277WAGKQUYfd79fef3d01d5e9aadc18ccd4d0c95071136; dplet=34d83874ac94a30a752042fc3c8246ee; dper=779316afd8d7defe2892e64b894b9d1bcec846b42e657f0f2a50f1fcbbd3ce84a8f72381ea817906deaade06de3edbcac79cdbbd0f1addc3a1a89c368e7bd986; ua=%E8%93%9D%E9%BB%91%E5%86%B0%E9%9B%A8; ctu=be0ff75e5fe65f75a87646e464f342aa9e7ac753dba9b4820aba17f3f9bfdfd4; aburl=1; Hm_lvt_602b80cf8079ae6591966cc70a3940e7=1689080595; Hm_lvt_6bc30b61e75e1c8b9c509994370bd6e9=1689080595; ll=7fd06e815b796be3df069dec7836c3df; _lxsdk_s=18952d21a18-8fb-20-1af%7C%7C2; qruuid=2819e9b5-f161-4273-bc17-fc31f18e572a',
    // 'Cookie': '',
}

myRequest(seedURL, "seed")

function myRequest(url, str){
    var options = {
        url: url,
        headers: headers,
        proxy: 'http://127.0.0.1:10809',
        timeout: 5000  // 最多等待5秒
    }
    Request(options, (err, response, body) => {
        if (!err && response.statusCode == 200) {
            console.log("request success:", url)  // 本来是utf8, 转码后反而乱码
            fs.writeFileSync("./htmls/"+url, body)
            // console.log(body)
            if(str == "seed") parseSeed(body)
            else if(str == "class") parseClass(body)
            else if(str == "shop")  parseShop(body, url)
        } else {
            console.log("request failed: " + err, response.statusCode, url)
        }
    })
}


function parseSeed(body) {
    let $ = Cheerio.load(body, { decodeEntities: true })  // 准备用cheerio解析html
    let all_class;
    try {
        all_class = $("#classfy a")
    } catch (e) { console.log('分类url获取出错:' + e) }

    all_class.each(function (index, elememt) {   // 遍历每个店家
        var href = ""   // 获取分类网站
        let class_name = ""  // 类型名称
        try {
            href = $(elememt).attr("href")
            class_name = $(elememt).text()
            // console.log(href, class_name)
            randomDelayRequest(href, "class");
        } catch (e) {
            // console.log('识别种子页面中的分类链接出错：' + e)
        }
    })
}

function parseClass(body){
    let $ = Cheerio.load(body, { decodeEntities: true })
    try {
        all_shop = $("#shop-all-list .tit>a")
    } catch (e) { console.log('店家简介块定位出错:' + e) }

    all_shop.each(function(index, elememt){
        var href = ""
        let shop_name = ""
        try{
            href = $(elememt).attr("href")
            randomDelayRequest(url, "shop");
            // console.log(href, shop_name)
        }catch(e){
            // console.log("店家链接获取错误："+e)
        }

    })
}

function parseShop(body, url){
    let $ = Cheerio.load(body, { decodeEntities: true })
    var url_shop = url
    var shop_name = ""
    var score = 0
    var leixing = ""
    var renjun = ""
    var dizhi = ""
    var pics = new Array()
    var _text = ""
    var _time = new Date()
    var fensi = 0
    var author_dizhi = ""
    var crawltime = new Date().toFormat("YYYY-MM-DD HH24:MI:SS")
    var laiyuan = "点评discovery"

    var fetchAddSql = "INSERT INTO dianping_discovery(_url,url_shop,shop_name,score,leixing,renjun,dizhi,pics,_text,_time,fensi,author_dizhi,crawltime,laiyuan) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
    var fetchAddSql_Params = new Array()
    try{
        shop_name = $(".shop-name").text().replace(/\s+/g, '').replace('手机扫码 优惠买单', '')
        score = $(".star-wrapper div").eq(1).text()
        leixing = $(".breadcrumb a").eq(1).text()
        qu = $(".breadcrumb a").eq(2).text()
        renjun = $("#avgPriceTitle").text()
        let regex = /(口味|服务|环境)：(\d+(\.\d+)?)/;
        kouwei = $("#comment_score .item:first").text().match(regex)[1]
        huanjing = $("#comment_score .item").eq(1).text().match(regex)[1]
        fuwu = $("#comment_score .item").eq(2).text().match(regex)[1]
        dizhi = $("#address").text()
        // dianhua = $(".expand-info tel").text()
        // yingyeshijian = $(".J-other span.item").eq(1).text()
        // let N_tuijiancai = $("ul.recommend-photo li")
        // N_tuijiancai.each((index, elememt) => {
        //     var cai = $(elememt).find("p").text()
        //     var price = $(elememt).find("span").text()
        //     tuijiancai[cai] = price
        // })
        _text = getComments(url)
        $(".photos").each((index, elememt) => {
            $(elememt).find(a).each((aindex, aelement) => {
                pics.push($(aelement).attr("href"))
            })
            if(pics.length > 32){
                return false;
            }
        })
        fetchAddSql_Params = [url, url_shop, shop_name, score, leixing, renjun, dizhi, pics.join("; "), _text, _time, fensi, author_dizhi, crawltime, laiyuan]
        console.log(fetchAddSql_Params)
    } catch(e) {console.log(url+" 解析出错：" +e)}
}

function getComments(url){
    return {4.5 : "henbang!", 2 : "不好吃。", 3 : "一般啊。"}
}

function delay(ms) {
    return new Promise(resolve => setTimeout(resolve, ms));
}

async function randomDelayRequest(url, str) {
    var minDelay = 500; // 最小停顿时间，单位：毫秒
    var maxDelay = 5000; // 最大0
    var delaysec = Math.floor(Math.random() * (maxDelay - minDelay + 1)) + minDelay;
    await delay(delaysec);

    myRequest(url, str)
    console.log("Delayed code executed!");
}

