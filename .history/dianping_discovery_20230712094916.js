...
function parseSeed(body) {
    ...
    all_class.each(function (index, elememt) {   // 遍历每个店家
        ...
        try {
            ...
            if (class_name == "推荐" || class_name == "推荐" || class_name == "推荐")
...
function parseItem(body, url){
    ...
    var fetchAddSql = "INSERT INTO dianping(url, shop_name, score, leixing, renjun, dizhi, pics, text, time, fensi, crawltime) VALUES(?,?,?,?,?,?,?,?,?,?,?)"
    ...
    try{
        ...
        fetchAddSql_Params = [url, shop_name, score, leixing, renjun, dizhi, pics.join(", "), text, time, fensi, crawltime]
