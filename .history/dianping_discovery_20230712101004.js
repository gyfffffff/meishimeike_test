...
function myRequest(url, str){
    ...
    Request(options, (err, response, body) => {
        if (!err && response.statusCode == 200) {
            ...
            if (!body.includes("内容加载失败") && !body.includes()){
...
function parseClass(body){
    ...
    for(var i=1; i<matches.length; i++){
        href = "https://www.dianping.com/discovery/"+matches[i]
    }
    randomDelayRequest(href, "item");
    // let $ = Cheerio.load(body, { decodeEntities: true })
    // try {
    //     all_item = $(".seed-waterfall-item")
    // } catch (e) { console.log('店家简介块定位出错:' + e) }

    // all_item.each(function(index, elememt){
    //     var href = ""
    //     try{
    //         href = "https://www.dianping.com"+$(elememt).find(a).attr("href")
    //         randomDelayRequest(href, "item");
    //     }catch(e){
...
function parseItem(body, url){
    ...
    var hot_comments = new Array()

    var fetchAddSql = "INSERT INTO dianping(_url, url_shop, shop_name, score, leixing, renjun, dizhi, pics, _text, hot_comments, _time, fensi, crawltime) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)"
    var fetchAddSql_Params = new Array()
    try{
        url_shop = $(".related-shop-item a").attr("href")
        shop_name = $(".shop-name").text()
        score = $(".sml-rank-stars").tagname.slice(-2)/10
        leixing = $(".area-info").text().split("|")[0].trim()
        dizhi = $(".area-info").text().split("|")[1].trim()
        renjun = $(".avg-info").text()
        _text = $(".content-module").text()
        $(".thumbnail-list img").each((index, elememt) => {
            pics.push($(elememt).attr("src"))
        })
        _time = $(".publish-time").text()
        fensi = $(".ugc-info-wrapper").text()-0
        $(".hotUGC-module-content-text").each((index, elememt) => {
            hot_comments.push($(elememt).text())
        })
        fetchAddSql_Params = [url, url_shop, shop_name, score, leixing, renjun, dizhi, pics.join(", "), _text, hot_comments, _time, fensi, crawltime]
...
async function randomDelayRequest(url, str) {
    var minDelay = 2000; // 最小停顿时间，单位：毫秒
    var maxDelay = 100000; // 最大停顿时间，单位：毫秒

    var delaysec = Math.floor(Math.random() * (maxDelay - minDelay + 1)) + minDelay;
    await delay(delaysec);

    myRequest(url, str)
    console.log("Delayed："+ delaysec/1000 + "毫秒");
}
