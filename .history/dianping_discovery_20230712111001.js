...
function parseSeed(body) {
    ...
    all_class.each(function (index, elememt) {   // 遍历每个店家
        ...
        try {
            ...
            if (class_name == "亲子"){  //class_name == "推荐" || class_name == "美食" ||
...
function parseItem(body, url){
    ...
    var fetchAddSql = "INSERT INTO dianping_discovery(_url, url_shop, shop_name, score, leixing, renjun, dizhi, pics, _text, hot_comments, _time, fensi, crawltime) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?.?)"  // 13个字段加一个zhujian，14个?
    ...
    try{
        ...
        fensi = $(".ugc-info-wrapper span").eq(2).text()-0
...
async function randomDelayRequest(url, str) {
    ...
    var maxDelay = 8000; // 最大停顿时间，单位：毫秒
