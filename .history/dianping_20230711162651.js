...
function myRequest(url, str){
    ...
    Request(options, (err, response, body) => {
        if (!err && response.statusCode == 200) {
            ...
            else if(str == "shop")  parseShop(body)
        } else {
...
function parseShop(body){
    ...
    var huanjing = ""
    var fuwu = ""
    var comments = {}  // score : text
    var pics = new Array()  // 25 img url, from comments

    var fetchAddSql = "INSERT INTO dianping(shop_name, score, leixing, renjun, dizhi, qu, dianhua, yingyeshijian, kouwei, huanjing, fuwu, comments, pics) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)"
    var fetchAddSql_Params = new Array()
    try{
        shop_name = $(".shop-name").text()
        score = $(".star-wrapper div:second").text()
        leixing = $(".breadcrumb a:second").text()
        qu = $(".breadcrumb a:3th").text()
        renjun = $("#avgPriceTitle").text()
        let regex = /(口味|服务|环境)：(\d+(\.\d+)?)/;
        kouwei = $("#comment_score item:first").text().match(regex)[1]
        huanjing = $("#comment_score item:second").text().match(regex)[1]
        fuwu = $("#comment_score item:3th").text().match(regex)[1]
        dizhi = $("#address").text()
        dianhua = $(".expand-info tel").text()
        yingyeshijian = $(".other J-other $('.info-indent span.item').eq(1).text().trim();")
