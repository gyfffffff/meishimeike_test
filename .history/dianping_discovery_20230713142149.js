...
function parseSeed(body) {
    ...
    all_class.each(function (index, elememt) {   // 遍历每个店家
        ...
        try {
            ...
            if (class_name == "宠物"){  //class_name == "推荐" || class_name == "美食" ||class_name == "生活服务" || class_name=="丽人"
...
function parseItem(body, url){
    ...
    var fetchAddSql = "INSERT INTO dianping_discovery(_url,url_shop,shop_name,score,leixing,renjun,dizhi,pics,_text,_time,fensi,crawltime,laiyuan) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)";  // 13个字段,13?
    ...
    try{
        ...
        fetchAddSql_Params = [url, url_shop, shop_name, score, leixing, renjun, dizhi, pics.join("; "), _text,  _time, fensi, crawltime,laiyuan]
        // console.log(fetchAddSql_Params)
        mysql.query(fetchAddSql, fetchAddSql_Params, (qerr, vals, fields) => {
            if(qerr){console.log(qerr)}
        })
    } catch(e) {console.log(url+" 解析出错：" +e, score, dizhi, fensi, body)}
}
